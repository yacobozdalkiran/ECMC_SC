...
void generate_metro(const RunParamsHbCB& rp, bool existing) {
    ...
    double epsilon = 0.15;
    int size = 1000;
    set = metropolis_set(epsilon, size, rng);

    size_t accepted{}, proposed{};
    //==============================ECMC Checkboard===========================
    // Thermalisation
    ...
    if (!existing) {
        ...
        for (int i = 0; i < rp.N_therm; i++) {
            for (int j = 0; j < h.N_sweeps; j++)
                metropolis::sweep(field, geo, h.beta, set, accepted, proposed, rng);
...
int main(int argc, char* argv[]) {
    ...
    generate_metro(params, existing);
